...
class Settings extends MY_Controller {
    ...
    public function list_permissions_ajax($page=0){
        ...
        if(isset($permissions) && $permissions!=NULL){
            ...
            foreach($permissions as $permission){
                ...
                $permissions_list[$i]['delivery_agent'] = $this->settings_model->get_role_permissions($role='delivery_agent',$permission_slug=$permission['permission_slug']);
                $permissions_list[$i]['inventory_guy'] = $this->settings_model->get_role_permissions($role='inventory_guy',$permission_slug=$permission['permission_slug']);
                $i++;
            }
        }
	...
	public function list_units()
	{
        ...
        if($this->permitted('list_users')){
            ...
            $data['units']=$units;
            $data['sub_view'] = $this->load->view('admin/units/list_units', $data, TRUE);
        }else{
            $data['sub_view'] = $this->load->view('errors/permission/denied', $data, TRUE);
        }
        $this->load->view('admin/_layout', $data); 
