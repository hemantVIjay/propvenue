...
class Localities_model extends MY_Model{
	...
	public function get_locations($id){
        ...
        $this->db->join('cities t', 'c.city_id = t.id', 'left');
        $this->db->group_by('c.id');
        if($id !=''){
	     $this->db->where('c.zone_id', $id);
		}
		$query = $this->db->get();
    ...
    //get all zoness
	public function get_zones($params = array()){
		$this->db->select('c.*');
	    ...
	    //get all zoness
	public function get_zoneDetail($id){
		$this->db->select('c.*');
        $this->db->from('zones c');
    ...
    public function create_location($post_data){
    ...
    //create new ticket category
    public function create_zlocation($post_data){
        $this->_table_name='locations';
        $this->_timestamps=TRUE;
        //create ticket caregory
        $insert_id=$this->save($data=$post_data, $id = NULL);
    ...
    //create new ticket category
    public function create_zone($post_data){
        $this->_table_name='zones';
        $this->_timestamps=TRUE;
        //create ticket caregory
        $insert_id=$this->save($data=$post_data, $id = NULL);
        if($insert_id){
